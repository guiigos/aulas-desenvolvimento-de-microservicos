...
const QUEUE = CONSUMER_QUEUES.CLASS_OFFERING_UPDATED;

@Injectable()
export class ClassOfferingUpdatedConsumer
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(ClassOfferingUpdatedConsumer.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    this.logger.log(
      `Consumer registrado na fila "${QUEUE}" (routing key "${CONSUMER_ROUTING_KEYS.CLASS_OFFERING_UPDATED}")`,
    );
  }
  ...
  private async handle(msg: ConsumeMessage | null): Promise<void> {
    ...
    if (!isClassOfferingUpdatedPayload(payload)) {
      this.logger.error(
...
function isClassOfferingUpdatedPayload(
  value: unknown,

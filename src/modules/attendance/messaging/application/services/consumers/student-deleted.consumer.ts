...
const QUEUE = CONSUMER_QUEUES.STUDENT_DELETED;

@Injectable()
export class StudentDeletedConsumer
  implements OnApplicationBootstrap, OnModuleDestroy
{
  private readonly logger = new Logger(StudentDeletedConsumer.name);
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    this.logger.log(
      `Consumer registrado na fila "${QUEUE}" (routing key "${CONSUMER_ROUTING_KEYS.STUDENT_DELETED}")`,
    );
  }
  ...
  private async handle(msg: ConsumeMessage | null): Promise<void> {
    ...
    if (!isStudentDeletedPayload(payload)) {
      this.logger.error(
    ...
    try {
      await this.studentRefRepository.deleteById(payload.id);
      this.channel.ack(msg);
    } catch (err) {
      this.logger.error(
        `Falha ao deletar student em "${QUEUE}" (id=${payload.id})`,
        err as Error,
      );
      this.channel.nack(msg, false, false);
    }
...
function isStudentDeletedPayload(value: unknown): value is { id: string } {
